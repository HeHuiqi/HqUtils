...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.titleNomalFont = [UIFont boldSystemFontOfSize:17];
        self.titleSelectedFont = [UIFont boldSystemFontOfSize:17];
        [self setupSubViews];
...
- (void)layoutAndScrollToSelectedItem {
    ...
    if (self.delegate) {
        [self.delegate categoryView:self selectIndex:self.selectedIndex];
    }
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        _collectionView.backgroundColor = HqRandomColor;
