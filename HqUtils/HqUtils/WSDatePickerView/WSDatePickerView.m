...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    for (int i=0; i<nameArr.count; i++) {
        ...
        CGRect rect  =CGRectMake(labelX, self.showYearView.frame.size.height/2-15/2.0, 15, 15);
        if (i == 0) {
            rect  =CGRectMake(labelX+10, self.showYearView.frame.size.height/2-15/2.0, 15, 15);
        }
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    if (!customLabel) {
        ...
        [customLabel setFont:[UIFont boldSystemFontOfSize:17]];
    }
    NSString *title;
